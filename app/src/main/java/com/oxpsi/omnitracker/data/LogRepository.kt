...
import com.oxpsi.omnitracker.ai.parseAiAnalysis
...
class LogRepository(private val context: Context, private val logDao: LogDao, private val recipeDao: RecipeDao) {    private val aiService = AiAnalysisService()
    ...
    suspend fun exportLite(startTime: Long? = null, endTime: Long? = null, filename: String = "omnitracker_lite"): File = withContext(Dispatchers.IO) {
    ...
    suspend fun exportFull(startTime: Long? = null, endTime: Long? = null, filename: String = "omnitracker_full"): File = withContext(Dispatchers.IO) {
        val logs = if (startTime != null && endTime != null) {
            logDao.getLogsInRange(startTime, endTime)
        } else {
            logDao.getAllLogsSnapshot()
